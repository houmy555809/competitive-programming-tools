...
WORKSPACE_HELP = "Use relative file paths to current workspace instead of the current working directory. Does not affect absolute paths."


def main():
    current_path = pathlib.Path(__file__).parent
    parser = argparse.ArgumentParser(prog = "cpt", description = "Competitive Programming Tools")
    ...
    parser_compare.add_argument("-w", "--workspace", action = "store_const", dest = "use_workspace", const = True, default = False, help = WORKSPACE_HELP)
    ...
    parser_cache_list = parser_cache_sub.add_parser("list", help = "List all saved cache files.")
    parser_cache_list.add_argument("-n", "--num", type = int, default = int(1e9), help = "Maximum numbers of cache directories to show")

    parser_cache_purge = parser_cache_sub.add_parser("purge", help = "Clear all cache files.")

    parser_dump = subparsers.add_parser("dump", help = "Dump cache files to a target directory.")
    parser_dump.add_argument("cache_id", nargs = "?", type = str, default = None, help = "Cache ID to dump (default: most recent)")
    parser_dump.add_argument("-o", "--output", type = str, default = ".", help = "Target directory to dump into (default: current directory)")
    parser_dump.add_argument("-w", "--workspace", action = "store_const", dest = "use_workspace", const = True, default = False, help = WORKSPACE_HELP)
    ...
    parser_workspace_set.add_argument("-w", "--workspace", action = "store_const", dest = "use_workspace", const = True, default = False, help = WORKSPACE_HELP)
